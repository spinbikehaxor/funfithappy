...
def lambda_handler(event, context):
    ...
    subject = html.escape(body['subject'].strip())
    messageText = html.escape(body['messageText'].strip())
    recipients = getClassRosterEmails(class_date)
...
def getClassRosterEmails(classdate):
    ...
    for i in query_response['Items']:
        username = i['username']
        email = getAttendeeEmail(username)
        roster.append(email)
        
    return roster
    
def getAttendeeEmail(username):
...
def sendMessage(recipients, subject, messageText):
    ...
    SUBJECT = subject
    CHARSET = "UTF-8"
    BODY_HTML = """<html><head></head><body>""" + messageText + """</body></html>"""
