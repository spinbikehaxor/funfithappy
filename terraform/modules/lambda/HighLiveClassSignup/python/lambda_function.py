...
def lambda_handler(event, context):
    ...
    is_active_subscriber = isSubscriber(username)
    ...
    spotNumber = signup(class_details['class_date'], capacity, isFree, class_type, is_active_subscriber)
    if(spotNumber == -1):
        return {
            'statusCode': 422,
            'headers': 
...
def signup(class_date, capacity, isFree, class_type, is_subscriber):
    ...
    #Only charge a credit if not on waitlist and not a subscriber
